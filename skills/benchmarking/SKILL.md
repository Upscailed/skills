...
name: benchmarking
title: "Benchmarking"
description: "Vergelijkt in 30-60 minuten de prestaties van een MKB-bedrijf tegen branchecijfers en/of directe concurrenten op minimaal 3 metrics. Output: een gestructureerd `Benchmarking.md` in `Directie/Research/` met per benchmark-punt eigen score, branche-bron met datum, gemiddelde, top-25%, top-10%, eigen positie (achter/op/voor), context en conclusie (sterkte, kans of actiepunt). Scant vooraf diep de vault op eigen KPI's (output van skills `operationele-kpis`, `financiele-data-setup`, `website-analytics-setup`, `klantfeedback-collector`, `marketing-data-setup`), concurrent-data (uit `Directie/Research/Concurrentie.md` van skill `concurrentie-research`, C6) en jaardoel-targets (`Directie/01 - Goals.md`, S5). Voert daarna actieve research uit naar branche-rapporten (CBS Statline, Eurostat, KvK-sectorrapporten, ECB sectoraal, McKinsey/PwC/Deloitte publicaties, SaaS-benchmarks zoals OpenView en Bessemer, agency-benchmarks). Vault-onafhankelijk, alleen relatieve paden binnen `{scope}/`. Activeer altijd wanneer iemand zegt: 'benchmark', 'benchmarking', 'vergelijking met concurrenten', 'branchecijfers', 'sector-vergelijking', 'hoe presteren wij ten opzichte van', 'A5 dichten', 'gap A5', 'benchmark-meting', 'periodieke vergelijking', 'gap-analyse t.o.v. branche', 'industry benchmark'. Triggert ook bij iedere expliciete verwijzing naar A5 of `Benchmarking.md`."
category: research
tags: [scale-framework, a5, analyze, benchmarking, concurrentie, branchecijfers, gap-analyse]
...
compatibility:
  ...
  note: "WebSearch en WebFetch zijn aanbevolen voor Stap 4 (actieve research naar branche-rapporten). Zonder die tools werkt de skill alleen op concurrent-data uit Concurrentie.md plus door de gebruiker aangeleverde rapporten. Voeg deze toe aan de allowlist via /update-config als ze nog niet zijn toegestaan."
---
