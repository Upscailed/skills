...
description: "Helpt een ondernemer in 30 tot 50 minuten een werkende Daily Business Briefing op te zetten: een korte, geautomatiseerde management-samenvatting die elke ochtend (06:00 tot 08:00, klant-instelbaar) verschijnt in inbox, Telegram-bot, Slack-channel, native app of als fysieke print. De skill is de operationalisering van de Upscailed Time-to-Value-belofte: al in week 1 een Daily Brief leveren, zodat de ondernemer vanaf dag 5 elke ochtend in 60 seconden weet hoe het bedrijf ervoor staat. De brief bevat een vaste indeling: datum plus dag, executive summary van 3 tot 5 zinnen (LLM-gegenereerd uit data), gisteren in 1 oogopslag (omzet, nieuwe leads, klantcontacten, issues), trend van afgelopen 7 dagen (pijl up of down per KPI), op de radar vandaag (deals die te sluiten zijn, klant-milestones, afspraken uit agenda), aandachtspunten en blockers, anomalieen (auto-gevlagd op afwijkingen >X% versus 7-daags-gemiddelde), en 1 concrete actie voor vandaag. Werkt vault-onafhankelijk en bouwt op data uit `Analyze/Rapportage.md` (A1, numerieke summaries), `Collect/CRM-setup.md` (C2, pipeline), `Collect/Financiele-data.md` (C3, omzet en cashflow), `Operations/KPIs.md` (C7, dagelijkse operationele KPIs), `Collect/Klantfeedback.md` (C4, NPS en CSAT) en `Collect/Marketing-data.md` (C1, leads en bereik). Output is een gevuld setup-document `{scope}/Analyze/Daily-Briefing-Setup.md` met content-spec, gekozen data-bronnen, gekozen kanaal, gekozen tijdstip, gekozen template-tone en een complete scheduled-task-specificatie die de ondernemer (of de Upscailed-onboarding-engineer) direct kan implementeren. Activeer altijd wanneer iemand zegt: 'daily briefing', 'ochtendrapport', 'dagelijkse cijfers', 'morning brief opzetten', 'A3 dichten in SCALE-audit', 'gap A3', 'Daily-Briefing-Setup.md aanmaken', 'hoe bouw ik een daily report', 'elke ochtend een dashboard in mijn inbox', 'time to value van Upscailed laten zien', 'cijfers per dag in mijn mailbox', 'ochtend-cijfers automatiseren', 'dagrapport per mail', 'Telegram-bot voor dagcijfers', 'Slack-bot ochtendcijfers', 'native app daily briefing'. Triggert ook bij iedere expliciete verwijzing naar A3 of `Daily-Briefing-Setup.md`."
category: operations
tags: [scale-framework, a3, analyze, daily, briefing, automation, daily-briefing, executive-summary, time-to-value]
