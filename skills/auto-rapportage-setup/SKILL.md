---
name: auto-rapportage-setup
title: "Auto-rapportage Setup"
description: "Helpt een ondernemer in 45 tot 90 minuten per rapportage-template uit A1 (`Analyze/Rapportage-Templates.md`) een werkende automatisering op te bouwen, vastgelegd in één canoniek document: `{scope}/Leverage/Auto-Rapportage.md` plus een aangevulde `{scope}/Operations/Automations.md`. Dit is SCALE-stap L2 (Leverage) en de directe operationalisering van A1. Waar A1 ontwerpt wat naar wie wanneer gaat, regelt L2 dat het zonder menselijke tussenkomst gebeurt: data ophalen via tool-API's of database-koppelingen (Stripe, Pipedrive, HubSpot, Moneybird, Plausible, Google Analytics, Notion, Airtable, custom DB), bewerken (filteren op periode, aggregeren naar KPI-niveau, indien nodig LLM-summarize voor de executive summary in 3 tot 5 zinnen), formatteren (HTML-template met inline CSS voor e-mail, PDF via puppeteer of weasyprint, Slack-blocks-payload, Telegram-Markdown-bericht, Notion-page-update), distributie (e-mail via Resend, Postmark, SendGrid of SMTP, Slack-webhook of Bot-API, Telegram-Bot-API, native-app-push via Expo, Notion-API), schedule (cron-expressie per rapport-frequentie), en failure-handling (retry-policy, alert-route bij faal, fallback naar handmatige versie zodat de eigenaar nooit zonder rapport zit). De skill werkt via een gestructureerd interview met 7 blokken plus een rapport-voor-rapport-deel: scope plus scan van Rapportage-Templates.md uit A1, tool-niveau kiezen (no-code Make of Zapier, low-code n8n self-hosted, of pro Node-script op Vercel-cron, Cloudflare Workers of GitHub Actions), per rapport de data-bron en pipeline-stappen vastleggen, distributie en kanaal-credentials, schedule-expressie afleiden uit de A1 tijdstippen, anomalie-detectie en alert-regels, AVG-overweging voor PII in geautomatiseerd verstuurde data, plus de scheduled-task-spec klaar om uit te rollen. Vóór het interview begint scant de skill diep wat al beschikbaar is in de vault: `Analyze/Rapportage-Templates.md` (A1, primaire input, zonder dit document raadt de skill aan eerst A1 te doen), de Collect-data (C1 t/m C7) voor bron-info, `IT/Toolstack.md` (S12) voor welke tools en accounts al in huis zijn, `HR/Team.md` voor eigenaren, `Analyze/Mission-Control.md` (A2) en `Analyze/Daily-Briefing-Setup.md` (A3) voor overlap-detectie. Werkt vault-onafhankelijk: geen aannames over een specifieke mapnaam of vault-eigenaar, alleen over de SCALE-conventie (`Leverage/`, `Analyze/`, `Operations/`, `IT/`, `HR/`). Activeer altijd wanneer iemand zegt: 'rapportage automatiseren', 'scheduled report', 'auto maandrapport', 'auto weekrapport', 'rapport vanzelf laten draaien', 'cron voor mijn rapport', 'rapportage in Make', 'rapportage in n8n', 'rapportage in Zapier', 'L2 dichten in SCALE-audit', 'gap L2', 'Leverage/Auto-Rapportage.md aanmaken', 'Operations/Automations.md bijwerken', 'Stripe-data naar mailrapport', 'Pipedrive weekrapport automatiseren', 'maandrapport via Resend versturen', 'PDF-rapportage genereren'. Triggert ook bij iedere expliciete verwijzing naar L2 of `Auto-Rapportage.md`."
category: operations
tags: [scale-framework, l2, leverage, rapportage, scheduled, automation, cron, integrations, no-code, low-code, pro-code]
