---
name: website-launch-check
title: "Website Launch Check"
description: "Helpt een ondernemer in 45 tot 90 minuten een website veilig en compleet live te zetten via een gestructureerde launch-checklist, opgeleverd in één canoniek bestand: `{scope}/Marketing/Website-Launch-Check.md`. Dit is SCALE-stap M1 (Marketing-aanwezigheid) en het verschil tussen een site die professioneel oogt en een site die op dag 1 al lekken vertoont: dode links, ontbrekende meta-tags, exposed `.env`-bestanden, geen cookiebanner, Lighthouse-score van 38, geen redirect van www, kapotte contactformulieren. De skill loopt 9 categorieën door: (1) Content (Brand Guidelines- en Tone of Voice-proof, geen lorem ipsum, contactinfo klopt), (2) SEO (meta-titles, descriptions, og-image, sitemap.xml, robots.txt), (3) Privacy/AVG (cookiebanner, privacyverklaring J4-link, AVG-checks), (4) Performance (Lighthouse > 80, images < 200kb, lazy-loading, Core Web Vitals groen), (5) Security (HTTPS-only, security headers, geen exposed `.env`), (6) Functioneel (CTA's, forms, 404/500-pages), (7) Analytics (C5 events live, UTM-discipline, conversie-tracking), (8) Pre-launch test (mobile/tablet/desktop, alle browsers, alle links, formulier-test, backup), (9) Domain + DNS (HTTPS-cert, www-redirect, SPF/DKIM voor mail). Pre-fill is diep: scant `{scope}/Marketing/` voor brand- en website-context, `{scope}/Directie/Playbook/Brand Guidelines.md` en `Tone of Voice.md` voor stijl-toets, `{scope}/Collect/Website-Analytics.md` voor C5 event-set, `{scope}/Juridisch/Privacyverklaring.md` voor J4-link, `{scope}/IT/Toolstack.md` voor stack (Vercel, Cloudflare, hosting), en haalt de site zelf op via web-fetch voor live-checks (meta-tags, status-code, HTTPS, headers, sitemap, robots.txt). Werkt vault-onafhankelijk: alleen relatieve paden binnen `{scope}`, geen hard-coded aannames. Activeer altijd wanneer iemand zegt: 'website launch check', 'site live zetten', 'pre-launch checklist', 'is mijn website klaar', 'check mijn website', 'website-launch readiness', 'site klaar voor publiek', 'go-live checklist website', 'launch readiness website', 'pre-launch audit', 'website-check voor productie', 'M1 dichten in SCALE-audit', 'gap M1', 'Marketing/Website-Launch-Check.md aanmaken', 'website-launch-check', 'staat alles goed op de site', 'kunnen we de site online zetten', 'wat moet er nog voor de launch'. Triggert ook bij iedere expliciete verwijzing naar M1, Website-Launch-Check.md, of vragen rond domein-, DNS-, HTTPS-, SEO-, performance- of cookiebanner-readiness."
category: operations
