---
name: dashboard-bouwer
title: "Dashboard Bouwer"
description: "Helpt een ondernemer in 60 tot 120 minuten een werkend custom dashboard te bouwen op basis van eigen data, vastgelegd in één canoniek bouw-spec: `{scope}/Engineer/Dashboard-Bouwer-Spec.md`. Dit is SCALE-stap E1 (Engineer) en de directe technische realisatie van A2 Mission Control-Ontwerp. Waar A2 ontwerpt welke 5 tot 9 hero-KPI's, 4 tot 6 trends, 5 tot 8 alerts, goal-progress-balken en recent-activity-feed op het scherm staan, regelt E1 hoe het ding daadwerkelijk gebouwd wordt: repo-scaffold (Vite-template of `npm create astro`), data-source-integratie (Stripe, Pipedrive, HubSpot, Moneybird, Plausible, GA4, Supabase, Postgres, Notion, Airtable), data-fetch-laag (server-side caching, cron-refresh via L2 of edge-fetch via SWR), visualisaties per tegel (KPI-card, line-chart, bar-chart, heatmap, sparkline, gauge), authenticatie (Clerk, Supabase-auth, eigen JWT), hosting (Vercel, Cloudflare Pages, Netlify, eigen VPS), monitoring (Sentry, BetterStack, UptimeRobot) en deploy-pijplijn (GitHub Actions, Vercel-preview). Stack-keuzes: Astro+React+Tailwind+Recharts is de Upscailed-default voor service-MKB, Vite+React+Tailwind+Recharts voor pure SPA, Next.js+shadcn+Tremor voor app-router-bedrijven, Notion-dashboard met embedded widgets voor no-code-route, Looker Studio voor Google-stack-bedrijven, Retool voor B2B-intern-tooling. De skill werkt via een gestructureerd interview met 8 bouw-blokken plus een tegel-loop: scope plus A2-check (`Analyze/Mission-Control-Ontwerp.md` verplicht aanwezig), stack-keuze, repo-scaffold, data-bron-integratie per tegel, visualisatie-keuze per tegel, auth-laag, hosting plus deploy, monitoring plus AIOS-in-a-Week-rol-out. Vóór het interview begint scant de skill diep wat al beschikbaar is: `Analyze/Mission-Control-Ontwerp.md` (A2, primaire input, verplicht aanwezig), `Collect/` voor data-bron-details, `IT/Toolstack.md` (S12) voor accounts en bestaande hosting-omgevingen, `HR/Team.md` voor wie wat ziet, `Leverage/Auto-Rapportage.md` (L2) voor overlap met scheduled-tasks die mogelijk dezelfde data al vullen. MKB-default is AIOS-in-a-Week: start in week 1 met 4 tot 6 hero-tegels live, breid uit per maand. Werkt vault-onafhankelijk, geen aannames over specifieke mapnaam of vault-eigenaar, alleen over de SCALE-conventie (`Engineer/`, `Analyze/`, `Collect/`, `IT/`, `HR/`, `Leverage/`). Activeer wanneer iemand zegt: 'dashboard bouwen', 'mission control bouwen', 'eigen dashboard-app', 'live dashboard met echte data', 'KPI-dashboard met API-data', 'custom dashboard ontwikkelen', 'CEO-dashboard live zetten', 'Mission Control implementeren', 'dashboard in Astro / Next.js / React', 'shadcn-dashboard', 'Tremor-dashboard', 'Looker Studio opzetten', 'Retool-app bouwen', 'Notion-dashboard met live widgets', 'AIOS-in-a-Week dashboard', 'E1 dichten in SCALE-audit', 'gap E1', 'Engineer/Dashboard-Bouwer-Spec.md aanmaken', 'A2 ontwerp gaan bouwen', 'mission-control-ontwerp implementeren'. Triggert NIET wanneer iemand het dashboard wil ONTWERPEN (gebruik A2 Mission Control Builder), een rapportage automatisch wil mailen (gebruik L2 Auto-rapportage Setup), een dagelijkse briefing wil opzetten (gebruik A3 Daily Briefing), een afdelingsrapport wil maken (gebruik A1 Rapportage Builder), een AI-tool of app van nul wil scopen (gebruik E2 App Scoping), of een data-pipeline tussen tools wil bouwen zonder UI (gebruik E3 Workflow Pipeline)."
category: bouwen
